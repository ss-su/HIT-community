...
export default class VPersonalAction extends React.Component<Props, State> {
  ...
  private load = async () => {
    const [err, data] = await to(personalApiService.ListUserCollection(appStore.userInfo.userId, 1, 999))
  ...
  private onViewNews = (NewsId: string,) => {
    this.props.history.push(`/news/detail/${NewsId}`)
  }
    ...
    return (
      <Wrapper>
        {data.map(
          ...
          i.articleShortDesc && (
            <AnswerContainer>
              <Title href='javascript:void(0);' onClick={this.onViewArticle.bind(this, i.id)}>
                【文章】{i.title}
              </Title>
              <TextContainer>
          ...
          i.newsShortDesc && (
            <AnswerContainer>
              <Title href='javascript:void(0);' onClick={this.onViewNews.bind(this, i.id)}>
                【新闻】{i.title}
              </Title>
              <TextContainer>
